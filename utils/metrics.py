...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir=".", names=(), eps=1e-16, prefix=""):
    ...
    # Find unique classes and remove class 2
    unique_classes, nt = np.unique(target_cls, return_counts=True)
    unique_classes = unique_classes[unique_classes != -1]
    nc = unique_classes.shape[0]  # number of classes, number of detections.
    
    # valid = (unique_classes != -1) & (unique_classes != 2) # !!
    # valid = (unique_classes != -1) & (unique_classes != 2) & (unique_classes != 3)
    # unique_classes = unique_classes[valid]
    # nt = nt[valid]

    nc = unique_classes.shape[0]  # number of valid classes
    ...
    i = smooth(f1.mean(0), 0.1).argmax()  # best F1 index
    p, r, f1 = p[:, i], r[:, i], f1[:, i]
    tp = (r * nt).round()
