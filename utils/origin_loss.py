...
class BCEBlurWithLogitsLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        dx = torch.clamp(dx, -10, 10)  # 값 범위 제한
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        p_t = torch.clamp(p_t, min=1e-7, max=1-1e-7)  # 0과 1 방지
        modulating_factor = (1.0 - p_t) ** self.gamma
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        lbox = torch.nan_to_num(lbox, nan=0.0)
        lobj = torch.nan_to_num(lobj, nan=0.0)
        lcls = torch.nan_to_num(lcls, nan=0.0)
